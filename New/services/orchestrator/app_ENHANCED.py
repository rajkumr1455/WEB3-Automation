...
async def run_scan_pipeline(scan_id: str, request: ScanRequest):
    ...
    try:
        ...
        if request.contract_address and not request.target_url:
            ...
            scans_db[scan_id]["current_stage"] = "address_scan"
            scans_db[scan_id]["progress"] = 5
            ...
            try:
                ...
                scans_db[scan_id]["results"]["address_scan"] = address_result
                
                if address_result.get("source_found"):
                    # Extract source code for static analysis
                    source_code_dict[address_result.get("contract_name", "Contract")] = {
                        "source": address_result.get("source_code"),
                        "abi": address_result.get("abi"),
                        "compiler_version": address_result.get("compiler_version")
                    }
                    logger.info(f"[{scan_id}] Source code retrieved from address scanner")
            except Exception as e:
                logger.error(f"[{scan_id}] Address scanner failed: {e}")
                # Continue with other stages even if address scan fails
        ...
        try:
            ...
            # Merge source code from address scanner if available
            if source_code_dict:
                recon_result.setdefault("source_code", {}).update(source_code_dict)
        except Exception as e:
            logger.warning(f"[{scan_id}] Recon failed, using address scanner results: {e}")
            recon_result = {"source_code": source_code_dict, "contracts": list(source_code_dict.keys())}
            scans_db[scan_id]["results"]["recon"] = recon_result
        ...
        # PRIORITY 3: Stage 5 - LLM-Enhanced Triage
        logger.info(f"[{scan_id}] Stage 5: LLM-Enhanced Triage")
        scans_db[scan_id]["current_stage"] = "triage"
        scans_db[scan_id]["progress"] = 80
        
        # First, run traditional triage
...
async def start_scan(request: ScanRequest, background_tasks: BackgroundTasks):
    ...
    logger.info(f"Started scan {scan_id} for {request.target_url or request.contract_address}")
